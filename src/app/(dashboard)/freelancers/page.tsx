  ...
  return (
    <div className="max-w-7xl mx-auto pb-20 space-y-10 animate-in fade-in duration-700">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-4xl font-black text-gray-900 tracking-tight">Freelancer Hub</h1>
          <p className="text-gray-400 font-medium mt-1">Verwalten Sie Arbeitszeiten, Projekte und Abrechnungen.</p>
        </div>
        
        {isSuperAdmin && (
          <div className="flex items-center gap-4 bg-white p-2 rounded-2xl shadow-sm border border-gray-100">
            <span className="text-xs font-bold text-gray-400 ml-2 uppercase tracking-widest">Ansicht für:</span>
            <select 
      ...
      {!selectedFreelancerId ? (
        <div className="text-center py-20 bg-white rounded-[40px] border border-gray-100 shadow-sm">
          ...
          <h3 className="text-xl font-bold text-gray-400">Kein Freelancer ausgewählt oder vorhanden.</h3>
        </div>
      ) : (
        <div className="space-y-8">
          ...
          <div className="flex bg-white p-2 rounded-[24px] shadow-sm border border-gray-100 w-fit">
            {[
              ...
              <button
                ...
                className={`flex items-center gap-2 px-6 py-3 rounded-[16px] font-bold text-sm transition-all ${
                  activeTab === tab.id 
          ...
          {activeTab === "dashboard" && (
            <div className="space-y-8">
              <div className="flex items-center justify-between">
                <h2 className="text-2xl font-black text-gray-900">Monatsübersicht</h2>
                <div className="flex items-center gap-4 bg-white border border-gray-100 rounded-xl p-1 shadow-sm">
                   <button onClick={handlePrevMonth} className="p-2 hover:bg-gray-50 rounded-lg transition-colors">
                     <ChevronLeft className="h-4 w-4 text-gray-400" />
                   </button>
                   <span className="font-bold text-gray-900 w-32 text-center text-sm uppercase tracking-widest">
                     {selectedMonth ? format(parseISO(`${selectedMonth}-01`), "MMM yyyy", { locale: de }) : "---"}
                   </span>
                   <button onClick={handleNextMonth} className="p-2 hover:bg-gray-50 rounded-lg transition-colors">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-gradient-to-br from-gray-900 to-black rounded-[32px] p-8 shadow-2xl relative overflow-hidden group">
                  <div className="absolute top-0 right-0 p-8 opacity-10 group-hover:scale-110 transition-transform duration-700">
                    <Clock className="h-32 w-32 text-white" />
                  </div>
                  <h3 className="text-gray-400 font-bold uppercase tracking-widest text-xs mb-2">Geleistete Stunden</h3>
                  <p className="text-6xl font-black text-white">{totalMonthHours.toFixed(1)} <span className="text-xl text-gray-500">h</span></p>
                </div>
                
                <div className="bg-gradient-to-br from-brand-secondary to-blue-600 rounded-[32px] p-8 shadow-2xl relative overflow-hidden group">
                   <div className="absolute top-0 right-0 p-8 opacity-10 group-hover:scale-110 transition-transform duration-700">
                    <Euro className="h-32 w-32 text-white" />
                  </div>
                  <h3 className="text-blue-200 font-bold uppercase tracking-widest text-xs mb-2">Umsatz (Brutto)</h3>
                  <p className="text-6xl font-black text-white">{totalMonthEarnings.toFixed(2)} <span className="text-xl text-blue-300">€</span></p>
                </div>
              </div>

              {/* Project Breakdown */}
              <div className="bg-white rounded-[40px] p-8 shadow-sm border border-gray-100">
                <h3 className="text-lg font-black text-gray-900 mb-6">Projektverteilung im {selectedMonth ? format(parseISO(`${selectedMonth}-01`), "MMMM yyyy", { locale: de }) : "ausgewählten Zeitraum"}</h3>
                {monthLogs.length === 0 ? (
                  <p className="text-gray-400 font-medium">Keine Einträge für diesen Monat vorhanden.</p>
                ) : (
                  <div className="space-y-4">
                    {Object.entries(
                      ...
                      <div key={pName} className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl">
                        ...
                        <div className="flex gap-8 text-right">
                          <div>
                            <p className="text-[10px] font-bold text-gray-400 uppercase">Stunden</p>
                            <p className="font-black text-gray-900">{stats.hours.toFixed(1)}h</p>
                          </div>
                          <div>
                            <p className="text-[10px] font-bold text-gray-400 uppercase">Umsatz</p>
                            <p className="font-black text-brand-secondary">€{stats.earnings.toFixed(2)}</p>
                          </div>
                        </div>
                      </div>
                    ))}
          ...
          {activeTab === "logs" && (
            <div className="space-y-6">
              <div className="flex justify-between items-center">
                <h2 className="text-2xl font-black text-gray-900">Arbeitseinträge</h2>
                <button 
                  onClick={() => {
                  ...
                  className="flex items-center gap-2 bg-brand-secondary text-white px-6 py-3 rounded-xl font-bold text-sm hover:scale-105 transition-all shadow-lg"
                >
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-gray-50 border-b border-gray-100">
                      ...
                      <th className="p-4 text-[10px] font-black uppercase tracking-widest text-gray-400 text-right">Maliyet (Kosten)</th>
                      ...
                      myLogs.map(log => {
                        ...
                        return (
                          <tr key={log.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                            ...
                            <td className="p-4 font-black text-brand-secondary text-right">
          ...
          {activeTab === "tags" && (
            <div className="space-y-6">
              <div className="flex justify-between items-center">
                <h2 className="text-2xl font-black text-gray-900">Leistungs-Tags & Stundensätze</h2>
                <button 
                  onClick={() => {
                  ...
                  className="flex items-center gap-2 bg-black text-white px-6 py-3 rounded-xl font-bold text-sm hover:scale-105 transition-all shadow-lg"
                >
                  ...
                  <div key={tag.id} className="bg-white rounded-[24px] p-6 shadow-sm border border-gray-100 relative group overflow-hidden">
                    ...
                    <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-all">
                      <button 
                        ...
                        className="h-8 w-8 bg-gray-50 rounded-full flex items-center justify-center text-gray-400 hover:bg-gray-200 hover:text-black"
                      >
                        <Edit2 className="h-4 w-4" />
                      </button>
                      <button 
                        onClick={() => deleteFreelancerTag(tag.id)}
                        className="h-8 w-8 bg-gray-50 rounded-full flex items-center justify-center text-red-400 hover:bg-red-500 hover:text-white"
                      >
                        <Trash2 className="h-4 w-4" />
                      </button>
                    </div>
                    ...
                    <h3 className="font-black text-gray-900 text-lg">{tag.name}</h3>
                    <p className="text-gray-400 font-bold mt-1 text-sm">{tag.hourlyRate.toFixed(2)} € / Stunde</p>
                  </div>
                ))}
                {myTags.length === 0 && (
                  <div className="col-span-full p-10 text-center border-2 border-dashed border-gray-200 rounded-[24px]">
                    <Tags className="h-10 w-10 mx-auto text-gray-300 mb-2" />
                    <p className="text-gray-400 font-bold">Noch keine Tags erstellt.</p>
                  </div>
                )}
              </div>
            </div>
      ...
      {isTagModalOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-md rounded-[32px] shadow-2xl p-8 relative animate-in zoom-in-95 duration-200">
            <h3 className="text-2xl font-black text-gray-900 mb-6">{editingTagId ? "Tag bearbeiten" : "Neuer Tag"}</h3>
      ...
      {isLogModalOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-xl rounded-[32px] shadow-2xl p-8 relative animate-in zoom-in-95 duration-200">
            <h3 className="text-2xl font-black text-gray-900 mb-6">{editingLogId ? "Arbeitseintrag bearbeiten" : "Neuer Arbeitseintrag"}</h3>
            ...
            <div className="space-y-4 mb-8">
              ...
              <div>
                ...
                <textarea 
                  ...
                  className="w-full bg-gray-50 border-none rounded-xl px-4 py-3 font-bold text-sm outline-none focus:ring-2 focus:ring-black/5 resize-none h-24"
            ...
            <div className="flex gap-3">
              ...
              <button 
                ...
                className="flex-1 py-3 bg-brand-secondary text-white rounded-xl font-black uppercase text-[10px] tracking-widest hover:bg-blue-700 transition-colors shadow-md"
              >
                Eintrag Speichern
              </button>
            </div>
